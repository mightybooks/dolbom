...
export interface ChoiceOption {
  ...
  label: string;
  type: CareType | "weird";
  isCoreRabbit?: boolean; // rabbit 고정 노출용
}
...
// OPTION RANDOMIZER
// 정상 5개 + 병맛 10개 중에서
// → rabbit normal 1개는 무조건 포함
// → 정상 중 rabbit 제외 4개 중 2개 랜덤
// → 병맛 중 10개 중 2개 랜덤
// 총 5개 노출
// ============================================================================

export function pickRandomOptions(all: ChoiceOption[]): ChoiceOption[] {
  ...
  // 정상 4 중 2 랜덤
  const normalShuffled = normalsWithoutRabbit.sort(() => Math.random() - 0.5);
  const pickedNormals = normalShuffled.slice(0, 2);

  // 병맛 10 중 2 랜덤
  const weirdShuffled = weirds.sort(() => Math.random() - 0.5);
  const pickedWeirds = weirdShuffled.slice(0, 2);
